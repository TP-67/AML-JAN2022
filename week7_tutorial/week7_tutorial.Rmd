...
```{r}
# Naive Bayes model
Naive_Bayes_basic = naive_bayes(x=train[ , -10],
                        y=as.factor(train$diagnosis_result), laplace=0 )
summary(Naive_Bayes_basic)

# Lapace Naive Bayes
Naive_Bayes_laplace = naive_bayes(x=train[ , -17],
...
```{r}
# y_train
pred_prob_training <- predict(Naive_Bayes_basic, type = 'class', train[ ,-10] )
# y_hat_train
...
```{r}
# y_train
pred_prob_laplace_training <- predict(Naive_Bayes_laplace, type = 'class', train[ ,-10] )
# y_hat_train
...
```{r}
# y_test
pred_prob_test <- predict(Naive_Bayes_basic, type = 'prob', test[ ,-10] )
pred_prob_test
# y_hat_test
pred_class_test = ifelse(pred_prob_test > 0.5, 1, 0)
# pred_class_test

test_enc = test %>% mutate(value = 1)  %>% spread(diagnosis_result, value,  fill = 0 )

t = cbind(test_enc$'0', test_enc$'1')

cm_test = table(t, pred_class_test)
cm_test
```
...
pred = prediction(pred_prob_test, t)
